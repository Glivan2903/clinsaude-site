...
const ENVIO_PREFIX = 'clinsaude:whatsapp:envio:';
...
const IA_UNIDADE_PREFIX = 'clinsaude:whatsapp:ia-unidade:';
...
const ENVIO_TTL_SECONDS = 30; // ver marcarEnvioEmAndamento/consumirEnvioRecente abaixo
...
// Se o paciente volta a escrever depois de ficar mais tempo que isso sem
// nenhuma atividade na conversa (nem dele, nem da Sofia/atendente), a Sofia
// cumprimenta de novo antes de continuar — sem esquecer o histórico. Ver
// SAUDACAO_GAP_MS em app/api/whatsapp/[unidade]/webhook/route.js.
export const SAUDACAO_GAP_MS = 6 * 60 * 60 * 1000; // 6 horas
...
function chaveEnvio(unidadeId, telefone) {
  return `${ENVIO_PREFIX}${unidadeId}:${telefone}`;
}
...
function chaveIaUnidade(unidadeId) {
  return `${IA_UNIDADE_PREFIX}${unidadeId}`;
}

// Interruptor geral por unidade — diferente da pausa por conversa
// (pausarConversa/definirIaAtiva abaixo, que valem só pra um telefone).
// Desligado aqui, a Sofia não responde a NINGUÉM naquela unidade, mesmo que
// a conversa individual esteja com iaAtiva:true. Sem TTL: fica do jeito que
// o admin deixou até ele mudar de novo. Default true (ativa) quando nunca
// foi definido.
export async function getIaAtivaUnidade(unidadeId) {
  const chave = chaveIaUnidade(unidadeId);
  const redis = getRedis();
  const valor = redis ? await redis.get(chave) : memoryGet(chave);
  // O cliente do Upstash desserializa "0" salvo como string de volta pra
  // number 0 na leitura — comparar direto com '0' (string) deixa passar
  // batido. String(valor) normaliza os dois formatos antes de comparar.
  return String(valor) !== '0';
}

export async function definirIaAtivaUnidade(unidadeId, ativa) {
  const chave = chaveIaUnidade(unidadeId);
  const redis = getRedis();
  const valor = ativa ? '1' : '0';
  if (redis) await redis.set(chave, valor);
  else memorySet(chave, valor, null);
}
...
export async function salvarHistorico(unidadeId, telefone, mensagens) {
  const chave = chaveConversa(unidadeId, telefone);
  let trimmed = (mensagens || []).slice(-HISTORY_MAX_MESSAGES);
  // O corte por tamanho pode deixar uma mensagem "tool" (resultado de
  // ferramenta) órfã no início — sem o "assistant" com tool_calls
  // correspondente antes dela, a OpenAI rejeita a conversa inteira na
  // próxima chamada. Descarta essas órfãs até sobrar um início válido.
...
// Fallback pro match por messageId acima: esse só existe DEPOIS que
// sendText() retorna, então se o eco da UAZAPI chegar (numa invocação
// serverless separada) antes dessa escrita terminar, consumirEcoEnvio não
// acha nada. Aqui marcamos "vai chegar um eco pra esse número" ANTES de
// chamar a UAZAPI — não depende de id nenhum, só de unidade+telefone.
export async function marcarEnvioEmAndamento(unidadeId, telefone) {
  const chave = chaveEnvio(unidadeId, telefone);
  const redis = getRedis();
  if (redis) {
    await redis.set(chave, '1', { ex: ENVIO_TTL_SECONDS });
  } else {
    memorySet(chave, '1', ENVIO_TTL_SECONDS);
  }
}

export async function consumirEnvioRecente(unidadeId, telefone) {
  const chave = chaveEnvio(unidadeId, telefone);
  const redis = getRedis();
  if (redis) {
    const existia = await redis.get(chave);
