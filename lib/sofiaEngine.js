...
// saudarNovamente.
//
// contexto: dados reais do canal (unidade/nome/telefone) repassados pra
// executeTool — hoje só usados por escalar_atendimento, pra ela não
// depender do modelo adivinhar ou perguntar de novo algo que o canal já
// sabe (ex.: o nome de exibição e o número do Whatsapp de quem está
// falando). undefined no chat do site, onde nada disso é conhecido de
// antemão.
export async function runSofiaTurn(
  historico,
  { today = new Date(), saudarNovamente = false, primeiraMensagem = false, contexto } = {}
) {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      for (const toolCall of message.tool_calls) {
        ...
        try {
          ...
          result = await executeTool(toolCall.function.name, args, contexto);
        } catch (err) {
