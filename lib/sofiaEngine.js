...
// saudarNovamente: true quando o chamador detectou que o paciente ficou um
// bom tempo sem mandar mensagem nesta conversa e voltou agora — o modelo não
// tem como perceber isso sozinho (as mensagens do histórico não carregam
// timestamp), por isso é um sinal explícito passado de fora (ver
// SAUDACAO_GAP_MS em lib/whatsappConversations.js).
export async function runSofiaTurn(historico, { today = new Date(), saudarNovamente = false } = {}) {
  ...
  try {
    ...
    if (saudarNovamente) {
      systemPrompt +=
        '\n\n[Contexto só desta resposta: o paciente ficou um bom tempo sem mandar mensagem nesta conversa e acabou de voltar. Cumprimente-o brevemente de novo antes de continuar (ex.: "Oi de novo! Vamos continuar de onde paramos?"), sem ignorar ou repetir o que já foi conversado antes.]';
    }
