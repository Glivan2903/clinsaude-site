...
import { sendText, getInstanciaUazapi, getInstanciasConfiguradas } from './uazapi';
...
// contexto vem de fora (webhook do Whatsapp ou chat do site — ver
// lib/sofiaEngine.js), nunca do modelo: unidade/nome/telefone já são
// conhecidos de verdade pelo canal (ex.: chatid e nome de exibição do
// Whatsapp), então confiar neles evita o modelo ter que perguntar de novo
// ou arriscar inventar esses dados. resumo/motivo são fornecidos pelo
// próprio modelo, que é quem tem o contexto da conversa pra sintetizar.
export async function escalar_atendimento({ resumo, motivo, unidade: unidadeSugerida } = {}, contexto = {}) {
  const numeroSuporte = process.env.SUPORTE_WHATSAPP_NUMERO;
  if (!numeroSuporte) {
    console.error('SUPORTE_WHATSAPP_NUMERO não configurado — não é possível escalar atendimento.');
    return { sucesso: false, mensagem: 'Não foi possível notificar um atendente agora.' };
  }

  const unidadeId =
    (contexto.unidade && getInstanciaUazapi(contexto.unidade) ? contexto.unidade : null) ||
    (unidadeSugerida && getInstanciaUazapi(unidadeSugerida) ? unidadeSugerida : null) ||
    getInstanciasConfiguradas()[0]?.id;

  if (!unidadeId) {
    console.error('Nenhuma instância de Whatsapp configurada — não é possível escalar atendimento.');
    return { sucesso: false, mensagem: 'Não foi possível notificar um atendente agora.' };
  }

  const texto = [
    `Unidade: ${nomeUnidade(contexto.unidade || unidadeId)}`,
    `Nome: ${contexto.nome || 'Não informado'}`,
    `Telefone: ${contexto.telefone || 'Não informado'}`,
    '',
    'Resumo da conversa:',
    clean(resumo) || 'Não informado.',
    '',
    `Observação: ${clean(motivo) || 'Não informado.'}`,
  ].join('\n');

  // Grupos do Whatsapp são identificados por um JID (ex.: "1203...@g.us"),
  // não por um número de telefone — onlyDigits removeria o sufixo "@g.us" e
  // quebraria o envio. Só normaliza pra dígitos quando for um contato comum.
  const numeroDestino = numeroSuporte.includes('@') ? numeroSuporte.trim() : onlyDigits(numeroSuporte);

  try {
    await sendText(unidadeId, { numero: numeroDestino, texto });
    return { sucesso: true, mensagem: 'Atendente notificado com sucesso.' };
  } catch (err) {
    console.error('Erro ao escalar atendimento para o suporte:', err);
    return { sucesso: false, mensagem: 'Não foi possível notificar um atendente agora.' };
  }
}
...
export const toolDefinitions = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        'Encaminha a conversa pra um atendente humano da clínica via Whatsapp interno. Use sempre que: (1) o paciente pedir explicitamente pra falar com um atendente/humano/pessoa/gerente, ou (2) você não tiver uma informação que o paciente precisa e nenhuma outra ferramenta resolve isso. Depois de chamar, avise o paciente de forma natural que um atendente vai dar continuidade por ali.',
      parameters: {
        type: 'object',
        properties: {
          resumo: {
            type: 'string',
            description:
              'Resumo curto (2-4 frases) do que o paciente já disse/pediu nesta conversa, pra o atendente humano entender o contexto rapidamente sem reler tudo.',
          },
          motivo: {
            type: 'string',
            description:
          ...
          unidade: {
            type: 'string',
            description:
              'Só preencha se souber com certeza (ex.: "matriz" ou "filial") e o canal não informar isso sozinho (chat do site). No Whatsapp, ignore este campo — a unidade já é resolvida automaticamente pelo número que o paciente chamou.',
          },
        },
...
// contexto: dados do canal (unidade/nome/telefone) que algumas ferramentas
// (hoje só escalar_atendimento) recebem além dos argumentos do modelo — ver
// lib/sofiaEngine.js. As demais ferramentas ignoram o segundo parâmetro.
