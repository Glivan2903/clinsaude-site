...
export default function AreaCliente() {
  ...
  useEffect(() => {
    const savedPhone = localStorage.getItem('clinSaude_patientPhone');
    if (savedPhone) {
  ...
  const handleLogout = () => {
    localStorage.removeItem('clinSaude_patientPhone');
    setPatient(null);
    setAppointments([]);
  ...
  const refreshHistory = async (phoneToQuery) => {
    const cleanPhone = phoneToQuery || patient?.CLI_CELULAR?.replace(/\D/g, '') || localStorage.getItem('clinSaude_patientPhone');
  ...
  const handleRescheduleSubmit = async () => {
    ...
    setRescheduleLoading(true);
    try {
      const cleanPhone = patient?.CLI_CELULAR?.replace(/\D/g, '') || localStorage.getItem('clinSaude_patientPhone');
      const payload = {
